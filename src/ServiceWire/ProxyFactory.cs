namespace ServiceWire
{
  ...
  public static class ProxyFactory
  {
    ...
    public static TInterface CreateProxy<TInterface>(Type channelType, Type ctorArgType, object channelCtorValue, ISerializer serializer)
    ...
    private static TInterface CreateProxy<TInterface>(ProxyBuilder proxyBuilder, object channelCtorValue, ISerializer serializer)
      where TInterface : class
    {
      //create the type and construct an instance
      var  ctorArgTypes    = new Type[] { typeof(Type), proxyBuilder.CtorType, typeof(ISerializer) };
      Type t               = proxyBuilder.TypeBuilder.CreateTypeInfo();
      var  constructorInfo = t.GetConstructor(ctorArgTypes);
      if (constructorInfo != null)
      {
        var instance = (TInterface)constructorInfo.Invoke(new object[] { typeof(TInterface), channelCtorValue, serializer });
        return instance;
      }
    ...
    private static ProxyBuilder CreateProxyBuilder(string proxyName, Type interfaceType, Type channelType, Type ctorArgType)
    {
      ...
      var ctorArgTypes = new Type[] { typeof(Type), ctorArgType, typeof(ISerializer) };
    ...
    private static MethodBuilder ConstructMethod(Type                     channelType,
                                                 MethodInfo               methodInfo,
                                                 TypeBuilder              typeBuilder,
                                                 Dictionary<Type, OpCode> ldindOpCodeTypeMap,
                                                 Dictionary<Type, OpCode> stindOpCodeTypeMap)
    {
      var paramInfos                                        = methodInfo.GetParameters();
      var nofParams                                         = paramInfos.Length;
      var parameterTypes                                    = new Type[nofParams];
      for (var i = 0; i < nofParams; i++) parameterTypes[i] = paramInfos[i].ParameterType;
      var returnType                                        = methodInfo.ReturnType;
      var methodBuilder =
        typeBuilder.DefineMethod(methodInfo.Name, MethodAttributes.Public | MethodAttributes.Virtual, returnType, parameterTypes);

      var mIL = methodBuilder.GetILGenerator();
      GenerateILCodeForMethod(channelType, methodInfo, mIL, parameterTypes, methodBuilder.ReturnType, ldindOpCodeTypeMap,
                              stindOpCodeTypeMap);
      return methodBuilder;
    }

    private static void GenerateILCodeForMethod(Type                     channelType,
                                                MethodInfo               methodInfo,
                                                ILGenerator              mIL,
                                                Type[]                   inputArgTypes,
                                                Type                     returnType,
                                                ...
                                                Dictionary<Type, OpCode> stindOpCodeTypeMap)
    {
      ...
      if (inputArgTypes.Length > 0)
      {
        ...
        for (var i = 0; i < inputArgTypes.Length; i++) args[i] = inputArgTypes[i].ToConfigName();
