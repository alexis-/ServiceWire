namespace ServiceWire.ZeroKnowledge
{
  ...
  public class ZkSession
  {
    ...
    public ZkCrypto Crypto => _zkCrypto;
    ...
    public bool ProcessZkProof(BinaryReader binReader, BinaryWriter binWriter, Stopwatch sw)
    {
      ...
      {
        _logger.Debug("ZkProof session hash does not match. Authentication failed. Server session hash: {0}",
                      Convert.ToBase64String(_serverSessionHash));
