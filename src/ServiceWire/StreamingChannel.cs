namespace ServiceWire
{
  ...
  public class StreamingChannel : Channel, IDvChannel
  {
    ...
    /// <summary>
    ///   This method asks the server for a list of identifiers paired with method names and
    ///   -parameter types. This is used when invoking methods server side.
    /// </summary>
                                          ...
                                          string password = null)
    {
      if (!string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(password))
      {
        ...
        {
          _logger.Debug("Server hash mismatch. InvalidCredentialException thrown. Has received: {0}",
                        Convert.ToBase64String(serverSessionHash));
          throw new InvalidCredentialException("authentication failed");
        }
        ...
        _logger.Debug("Zk authentication completed successfully.");
      ...
      if (!SyncInfoCache.TryGetValue(serviceType, out _syncInfo))
      {
        ...
        {
          // Ex: "ServiceWireTests.TestServices.INetTesterV1, ServiceWireTests"
    ...
    protected override object[] InvokeMethod(string metaData, params object[] parameters)
    {
      ...
      lock (_syncRoot)
      {
        ...
        for (var index = 0; index < _syncInfo.MethodInfos.Length; index++)
        {
        ...
        if (ident < 0)
          throw new Exception($"Cannot match method '{mdata[0]}' to its server side equivalent");
        ...
        var methodSyncInfo = _syncInfo.MethodInfos[ident];
